...
class MoodDatabaseManager: ObservableObject {
    ...
    /// Search for mood entries with optional filters
    /// - Parameters:
    ///   - startDate: Optional start date for filtering entries
    ///   - endDate: Optional end date for filtering entries
    ///   - moodType: Optional mood type for filtering entries
    ///   - keyword: Optional keyword to search in notes
    /// - Returns: Array of MoodEntryEntity objects matching the filters
    func searchMoodEntries(
        startDate: Date? = nil,
        endDate: Date? = nil,
        ...
        // Create an array to hold all predicates
        var predicates: [NSPredicate] = []
        
        // Add date range predicate if dates are provided
        ...
        // Add mood type predicate if a mood is provided
        if let moodType = moodType {
            predicates.append(NSPredicate(format: "moodEmoji == %@", moodType.rawValue))
        }
        
        // Add keyword search predicate if a keyword is provided
        if let keyword = keyword, !keyword.isEmpty {
            // Search in notes (case and diacritic insensitive)
            predicates.append(NSPredicate(format: "note CONTAINS[cd] %@", keyword))
        }
        
        // Combine all predicates with AND logic if there are any
        ...
        do {
            ...
            print("Error searching mood entries: \(error)")
            return []
        }
    }
